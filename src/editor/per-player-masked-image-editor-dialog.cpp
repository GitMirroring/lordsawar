...
#define method(x) sigc::mem_fun(*this, &PerPlayerMaskedImageEditorDialog::x)

const int PerPlayerMaskedImageEditorDialog::MAX_IMAGES_WIDTH = 1000;

PerPlayerMaskedImageEditorDialog::PerPlayerMaskedImageEditorDialog(Gtk::Window &parent, Glib::ustring filename, std::vector<PixMask *>image, std::vector<PixMask *>mask, double ratio, Shieldset *shieldset)
 : LwEditorDialog(parent, "per-player-masked-image-editor-dialog.ui")
{
  ...
  for (guint32 i = 0; i < image.size (); i++)
    d_image.push_back (image[i]->copy ());
  for (guint32 i = 0; i < mask.size (); i++)
    d_mask.push_back (mask[i]->copy ());
...
PerPlayerMaskedImageEditorDialog::~PerPlayerMaskedImageEditorDialog()
{
  for (guint32 i = 0; i < d_image.size (); i++)
    {
      if (d_image[i])
        delete d_image[i];
      if (d_mask[i])
...
bool PerPlayerMaskedImageEditorDialog::load_image ()
{
  bool broken = false;
  PixMask *p = PixMask::create (d_target_filename, broken);
  if (!broken)
    {
      d_image = disassemble_row(p->to_pixbuf (), MAX_PLAYERS, true);
      d_mask = disassemble_row(p->to_pixbuf (), MAX_PLAYERS, false);
      delete p;
    }
...
int PerPlayerMaskedImageEditorDialog::run()
{
  show_image();
...
void PerPlayerMaskedImageEditorDialog::hide()
{
  dialog->hide();
}

void PerPlayerMaskedImageEditorDialog::on_image_chosen(Gtk::FileChooserDialog *d)
{
...
void PerPlayerMaskedImageEditorDialog::update_panel()
{
  ...
  clear_button->set_visible (d_image.empty () == false);
}

void PerPlayerMaskedImageEditorDialog::show_image()
{
  if (d_image.empty () == true)
    {
      image_white->clear();
      image_green->clear();
      image_yellow->clear();
  ...
  Vector<int> dim = Vector<int>(d_image[0]->get_width(), d_image[0]->get_height());
  if (dim.x * MAX_PLAYERS  > MAX_IMAGES_WIDTH)
    {
      dim.x = MAX_IMAGES_WIDTH / MAX_PLAYERS;
      dim.y = d_image[0]->get_height() *
        (double)((double)dim.x / (double)d_image[0]->get_width());
    }
  for (unsigned int i = Shield::WHITE; i < Shield::NEUTRAL; i++)
	...
	default : break;
      ...
      PixMask *p = ImageCache::applyMask(d_image[i], d_mask[i], colour);
...
Gtk::FileChooserDialog* PerPlayerMaskedImageEditorDialog::image_filechooser(bool clear)
{
...
void PerPlayerMaskedImageEditorDialog::on_imagebutton_clicked ()
{
  Gtk::FileChooserDialog *d = image_filechooser(d_image.empty () == false);
  int response = d->run();
  ...
  else if (response == Gtk::RESPONSE_REJECT && d_image.empty () == false)
    clear_button->activate ();
  d->hide();
  delete d;
}

void PerPlayerMaskedImageEditorDialog::setup_shield_theme_combobox(Gtk::Box *box)
{
...
void PerPlayerMaskedImageEditorDialog::on_shieldset_changed()
{
  show_image();
}
