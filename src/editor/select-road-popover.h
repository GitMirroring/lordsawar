...
public:
    ...
    void setup (Road *road)
      {
        Gtk::ToggleButton *first = NULL;
        Gtk::ToggleButton *selected = NULL;

        std::vector<Road::Type> types = Road::getTypes ();
    ...
    sigc::signal<void(Road::Type)> signal_road_selected ()
      {
        return m_signal_road_selected;
      }
...
private:
    ...
    sigc::signal<void(Road::Type)> m_signal_road_selected;
    ...
    PixMask* make_pic (Road::Type t)
      {
        ...
        PixMask *r = ts->getRoad ()->getImage (t);
        if (r)
