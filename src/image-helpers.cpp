...
//  Copyright (C) 2009, 2011, 2014, 2021, 2026 Ben Asselstine
//
...
{
  auto paintable = source->get_paintable ();
  auto* new_image = Gtk::make_managed<Gtk::Image>(paintable);
  new_image->set_pixel_size (source->get_pixel_size ());
  return new_image;
}

void cairo_surface_to_drawing_area (Cairo::RefPtr<Cairo::Surface> map,
                                    Gtk::DrawingArea *area)
{
  ...
  area->set_draw_func
    ([img_surface] (const Cairo::RefPtr<Cairo::Context>& cr, int w, int h)
     {
       (void) w;
       (void) h;
