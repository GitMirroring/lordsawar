...
class AddProfileDialog: public LwDialogBase
{
public:
    static std::string get_resource_name ()
      {
        return "add-profile.ui";
      }

    AddProfileDialog (BaseObjectType* o,
                      const Glib::RefPtr<Gtk::Builder>& xml)
      : LwDialogBase (o, xml)
      {
        m_close_button = load <Gtk::Button> ("close_button");
        m_accept_button = load <Gtk::Button> ("accept_button");
        m_nick_entry = load <Gtk::Entry> ("nick_entry");
      }
    ...
    void setup ()
      {
        ...
        signal_response ().connect
          ([this](Gtk::ResponseType resp)
           {
             auto name = String::utrim (m_nick_entry->get_text ());
             switch (resp)
               {
               case Gtk::ResponseType::ACCEPT:
                 if (is_valid_name (name))
                   Profilelist::instance ()->push_back (new Profile (name));
                 break;
               default:
                 break;
               }
